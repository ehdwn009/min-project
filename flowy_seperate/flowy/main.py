...
# from flowy.routers import stt, summarization, action_assignment, feedback # <--- 이 줄을 아래로 이동

flowy_state: dict[str, Any] = {}

@asynccontextmanager
async def lifespan(flowy: FastAPI):
    # ... (lifespan 함수 내용은 이전과 동일) ...
    print("애플리케이션 시작: 리소스를 초기화합니다...")
    # ... (OpenAI 클라이언트 및 STT 파이프라인 초기화 로직) ...
    print("  OpenAI 클라이언트 초기화 중...")
    try:
        flowy_state["openai_client"] = OpenAI(api_key=settings.OPENAI_API_KEY)
        print("  OpenAI 클라이언트 초기화 완료.")
    except Exception as e:
        print(f"  OpenAI 클라이언트 초기화 실패: {e}")
        flowy_state["openai_client"] = None
...
def get_openai_client() -> OpenAI:
    client = flowy_state.get("openai_client")
    if client is None:
        raise HTTPException(status_code=503, detail="OpenAI 서비스 사용 불가 (초기화 실패)")
    return client

def get_stt_pipeline() -> Any:
    pipeline = flowy_state.get("stt_pipeline")
    if pipeline is None:
        raise HTTPException(status_code=503, detail="STT 서비스 사용 불가 (모델 로드 실패)")
    return pipeline

# --- 라우터 임포트 (모든 주요 객체 및 함수 정의 이후) ---
from flowy.routers import stt, summarization, action_assignment, feedback # <--- 위치 변경

flowy.include_router(stt.router, prefix="/api/stt", tags=["1. 음성-텍스트 변환 (STT)"])
flowy.include_router(summarization.router, prefix="/api/summarization", tags=["2. 텍스트 요약"])
flowy.include_router(action_assignment.router, prefix="/api/action-assignment", tags=["3. 역할 및 할 일 분배"])
flowy.include_router(feedback.router, prefix="/api/feedback", tags=["4. 회의 내용 피드백"])

@flowy.get("/", tags=["Root"])
async def read_root():
    # ... (이전과 동일) ...
    stt_status = "사용 가능" if flowy_state.get("stt_pipeline") else "사용 불가 (초기화 실패 또는 진행 중)"
    openai_status = "사용 가능" if flowy_state.get("openai_client") else "사용 불가 (초기화 실패 또는 진행 중)"
    return {
